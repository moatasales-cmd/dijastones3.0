...
export async function pageMeta({
  ...
  const rel = path === "/" ? "" : path;
  // Canonical points at THIS locale's own URL; hreflang alternates connect
  // all language variants (middleware.ts serves the prefixed ones).
  const url = `${SITE_URL}${localePrefix(locale)}${rel}` || SITE_URL;
  const languages = noIndex
    ? undefined
    : {
        ...Object.fromEntries(
          locales.map((l) => [l, `${SITE_URL}${localePrefix(l)}${rel}` || SITE_URL])
        ),
